...
def Moran_I(genes_exp,x, y, k=6, model="KNN", rad_cutoff=150):
    ...
    if model=="KNN":
        ...
        XYnbrs = NearestNeighbors(radius=rad_cutoff).fit(XYmap)
        XYdistances, XYindices = XYnbrs.radius_neighbors(XYmap, return_distance=True)
        W = np.zeros((genes_exp.shape[0],genes_exp.shape[0]))
        for i in range(0,genes_exp.shape[0]):
            W[i,XYindices[i]]=1
        for i in range(0,genes_exp.shape[0]):
...
def Geary_C(genes_exp,x, y, k=6, model="KNN", rad_cutoff=150):
    assert(model in ['Radius', 'KNN'])
    XYmap=pd.DataFrame({"x": x, "y":y})
    if model=="KNN":
        XYnbrs = NearestNeighbors(n_neighbors=k, algorithm='auto',metric = 'euclidean').fit(XYmap)
